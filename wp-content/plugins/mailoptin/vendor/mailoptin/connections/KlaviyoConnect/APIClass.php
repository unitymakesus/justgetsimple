...
class APIClass
{
     ...
     * @param string $confirm_optin true|false
     * @param array $properties extra data to tie to the subscriber
     *
     * @return array
     * @throws \Exception
     */
    public function add_subscriber($list_id, $email, $first_name = '', $last_name = '', $properties = [], $confirm_optin = 'false')
    {
        /** @var array $payload_properties @see https://www.klaviyo.com/docs/http-api#people eg there is $phone_number */
        $payload_properties = array_replace(['$first_name' => $first_name, '$last_name' => $last_name], $properties);

        $payload = ['email' => $email, 'confirm_optin' => $confirm_optin];

        if (!empty($payload_properties)) {
            $payload['properties'] = json_encode($payload_properties);
        }
        ...
        $response = $this->make_request("list/$list_id/members", $payload, 'post');
