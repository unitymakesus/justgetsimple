...
class Subscription extends AbstractConvertFoxConnect
{
    ...
    public function record_event()
    {
        ...
        if (parent::is_http_code_not_success($result['status_code'])) {
            self::save_optin_error_log($this->email . ': ' . json_encode($result['body']->errors), 'convertfox');
        }
    ...
    public function subscribe()
    {
        try {
            ...
            self::save_optin_error_log(json_encode($response['body']), 'convertfox');
            return parent::ajax_failure(__('There was an error saving your contact. Please try again.', 'mailoptin'));

        } catch (\Exception $e) {
            self::save_optin_error_log($e->getCode() . ': ' . $e->getMessage(), 'convertfox');
            return parent::ajax_failure(__('There was an error saving your contact. Please try again.', 'mailoptin'));
        }
    }
}